...
;**************************** Padding and Signature **********************************
      ...
      times 446-($-$$) db 0   ; $-$$ means current position minus the starting position, which gives the code size at this point. 446 is the endpoint of the zeros "times" put


;here we define the partition table entry

db 0x80       ;an indicator 0x80 at byte 446,  meaning bootable/active               
db 0x0          ; head number
dw 0000001000000000   ;sector number, cylinder
db 0x87    ;NTFS system ID
db 0x255     ; end head number
dw 0111111111111111    ;end sector number,  ending cylinder
dd 0x63        ;relative sector number
dd 0x63       ; number of sectors

code_size equ ($-$$)     ;code size at this points

times 510-(code_size) db 0      ; fill the sector from the partition table entry above to 510 with zeros.
